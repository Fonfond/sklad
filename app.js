...
const SPREADSHEET_ID = '1zqf4QEZGhmFgb1uOAe_F3DyckGkgZeN62UhmUNxuEFA'; // ВАШ ID
const API_KEY = 'AIzaSyAGNT2CQQzmyU5Hz4Z6hcE34LtHE10H_yc'; // ВАШ API КЛЮЧ
// ==================================================================
...
let scannedItemsBuffer = []; // Буфер для отсканированных позиций
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Попытка синхронизации при загрузке и каждые 30 секунд
    syncData();
    setInterval(syncData, 30000);
    
    // Регистрация Service Worker для полного офлайна
...
async function syncData() {
    ...
    try {
        // 1. Тянем данные из Google Таблиц в локальную БД
        // Загружаем Товары
        ...
        if (itemsData.values && itemsData.values.length > 1) {
            for (let i = 1; i < itemsData.values.length; i++) {
                ...
                // Кладем в локальную БД с upsert (обновить или вставить)
                try {
        ...
        // Загружаем Журнал (только для чтения в офлайне, пишем через буфер)
        ...
        // Загружаем Сотрудников (лист "Сотрудники")
        try {
            ...
            console.warn('Не удалось загрузить лист "Сотрудники":', e);
        }

        // 2. Отправляем все, что накопилось в локальной очереди на запись
        const pending = await localDB.allDocs({ include_docs: true, key: 'pending' });
        // Механизм записи в Google Таблицы через очередь (упрощен)
        // Реальная запись через Google Sheets API сложнее и требует OAuth, 
        // поэтому используем локальный буфер и ручной экспорт.
...
// Отображение списка товаров (Вкладка Товары и Взять)
async function renderStockList() {
    ...
    container.innerHTML = html || 'Склад пуст';
    
    // Обновляем также список "Взять"
    renderTakeList();
}
...
function renderTakeList() {
    ...
    localDB.allDocs({ include_docs: true }).then(result => {
        ...
        container.innerHTML = html || 'Ничего не найдено';
    });
}
...
async function takeItem(docId) {
    ...
    try {
        ...
        // Помечаем для отправки в Google Таблицы
        ...
        alert(`✅ ${doc.name} x${qty} списано. Облако обновится при связи.`);
        renderStockList();
        renderJournal();
    } catch (err) {
...
async function renderJournal() {
    ...
    const result = await localDB.allDocs({ include_docs: true });
    const journal = result.rows.filter(row => row.doc.type === 'journal').sort((a, b) => b.doc.time.localeCompare(a.doc.time));
    ...
    container.innerHTML = html || 'Журнал пуст';
}

// ================== 3. РАСПОЗНАВАНИЕ ФОТО (ОФЛАЙН) ==================
document.getElementById('cameraInput').addEventListener('change', async (e) => {
    ...
    try {
        // Tesseract.js работает полностью офлайн, если один раз загрузил языки
        const worker = await Tesseract.createWorker('rus');
        const { data: { text } } = await worker.recognize(file);
        await worker.terminate();
        
        // Простой парсинг строк. Предполагаем формат: "Наименование Количество"
...
async function saveScannedData() {
    for (let i = 0; i < scannedItemsBuffer.length; i++) {
        const name = document.getElementById(`scanName_${i}`).value;
        const qty = parseFloat(document.getElementById(`scanQty_${i}`).value) || 1;
        
        if (!name) continue;
        
        // Ищем товар по имени или создаем новый
        const result = await localDB.allDocs({ include_docs: true });
